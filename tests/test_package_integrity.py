...
class TestPackageIntegrity:
    ...
    def test_zip_extraction_succeeds(self, instrument, tmp_path):
        ...
        try:
            ...
            assert data_path.is_dir(), f"Test data path is not a directory for {instrument}"
    ...
    def test_metronix_extracted_structure(self):
        ...
        assert northern_mining.exists(), "Northern_Mining directory not found after extraction"
    ...
    def test_phoenix_extracted_structure(self):
        ...
        bin_files = list(sample_data.glob("*.bin"))
    ...
    def test_xml_metadata_files_present(self, package_root):
        ...
        for xml_dir in xml_dirs:
            assert xml_dir.exists(), f"Missing XML directory: {xml_dir.relative_to(package_root)}"
            xml_files = list(xml_dir.glob("*.xml"))
            assert len(xml_files) > 0, f"No XML files in {xml_dir.relative_to(package_root)}"
